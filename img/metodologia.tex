...
\node[phaseBuild, below=of f4] (f5) {\textbf{Fase 5:} Integração dos módulos ao \textit{framework}};
\node[phaseEval, below=of f5] (f6) {\textbf{Fase 6:} Avaliação empírica em modelos reais};
\node[phaseSynth, below=of f6] (f7) {\textbf{Fase 7:} Documentação, análise e síntese dos resultados};
...
\node[output, right=0.3cm of f5.east] {Saída: arquitetura integrada\\(\textit{Input Generator} $\rightarrow$ \textit{Evaluation Component})};
\node[output, right=0.3cm of f6.east] {Saída: métricas, falhas detectadas\\e comparação entre provedores};
\node[output, right=0.3cm of f7.east] {Saída: conclusões, recomendações\\e evidências consolidadas};
