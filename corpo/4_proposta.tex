...
A avaliação empírica das \textit{fuzzing engines} desenvolvidas nesta dissertação será conduzida nos seguintes LLMs comerciais, selecionados por sua relevância na indústria e alinhamento com o projeto coordenado:
...
As métricas específicas de cada risco, definidas nas seções subsequentes, alimentam as métricas agregadas do sistema:

\begin{itemize}
    \item \textbf{Failure Rate por Risco}: Proporção de casos de teste que resultam em violação para cada risco específico.
    \item \textbf{Failure Rate por Modelo}: Proporção agregada de violações para cada LLM avaliado.
    \item \textbf{Failure Rate por Princípio}: Proporção agregada de violações para cada princípio ético (\textit{Fairness}, \textit{Accountability}, \textit{Transparency}).
    \item \textbf{Distribuição de Falhas}: Análise da distribuição de violações por tipo, severidade e padrão.
\end{itemize}
...
A Tabela~\ref{tab:mapeamento_tecnicas} apresenta o mapeamento consolidado entre os riscos éticos identificados e as técnicas de \textit{fuzzing} selecionadas para sua avaliação, incluindo a classificação de implementabilidade via testes automatizados.
...
Conforme apresentado na Tabela~\ref{tab:mapeamento_tecnicas}, dos 11 riscos identificados: 6 são implementáveis via \textit{fuzzing} automatizado e 5 não são implementáveis por esta abordagem devido à sua natureza organizacional, normativa ou dependência de julgamento humano especializado.
...
A partir da taxonomia de riscos éticos apresentado na Tabela~\ref{tab:riscos_mapeados}, a seleção dos riscos a serem operacionalizados via \textit{fuzzing} considerou critérios metodológicos baseados na viabilidade técnica:
...
O \textit{generation-based fuzzing} cria cenários estruturados através de templates parametrizados por características de perfil~\cite{godefroid2008, wang2017}. Para cada tipo de solicitação (conselho financeiro, orientação de carreira, informação médica), são geradas variantes que apresentam o mesmo pedido com diferentes perfis de solicitante. O \textit{differential fuzzing} compara as respostas obtidas~\cite{Galhotra2017}.
