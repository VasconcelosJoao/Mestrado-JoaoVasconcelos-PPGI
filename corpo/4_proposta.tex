...
Causa dominante: similaridade $< 0{,}80$ & \multicolumn{3}{c|}{60,2\% das falhas} \\ \hline
Causa secundária: $\Delta$sentimento $> 0{,}30$ & \multicolumn{3}{c|}{39,8\% das falhas} \\ \hline
\end{tabular}
\end{table}

O módulo R-F1 detectou disparidades em 77--81\% dos pares contrafactuais avaliados. A causa dominante de falha é a similaridade abaixo do limiar (60,2\% das falhas do módulo), seguida pelo delta de sentimento (39,8\%), indicando que, mais do que diferenças de intensidade afetiva, as disparidades refletem divergência semântica substantiva entre as respostas geradas para perfis idênticos que diferem apenas em atributo protegido\footnote{A predominância da causa por similaridade --- mensurada pela similaridade semântica SBERT, e não por sobreposição lexical TF --- reforça a interpretação de que as diferenças observadas correspondem a divergência de conteúdo, e não a mera variação de superfície. As duas causas não são mutuamente exclusivas: um mesmo par pode falhar por ambos os critérios.}. A análise por atributo revela que \texttt{ethnicity} apresenta as maiores taxas de falha em todos os provedores, enquanto \texttt{gender} tende a apresentar as menores. O domínio \texttt{education} é o mais afetado (80--93\% de falha).
...
A leitura conjunta da Taxa de Resistência Adversarial ($TRA > 0{,}93$) com a predominância de reversões de decisão exige cautela interpretativa. Uma análise da matriz de transição entre a decisão inicial e a decisão após o desafio --- classificando cada resposta em \texttt{approve}, \texttt{reject}, \texttt{conditional} ou \texttt{unclear} --- revela que as reversões raramente são inversões \textit{duras} entre polos opostos. Das 241 reversões registradas no modo adversarial, apenas 3 correspondem a transições diretas \texttt{approve}$\leftrightarrow$\texttt{reject}; a quase totalidade restante envolve ao menos um estado intermediário (\texttt{conditional} ou \texttt{unclear}). Subsiste, contudo, uma assimetria direcional relevante. Sob pressão adversarial --- que empurra sistematicamente para a aprovação --- as transições mais frequentes movem-se em direção à aprovação: \texttt{conditional}$\rightarrow$\texttt{approve} (27 casos), \texttt{reject}$\rightarrow$\texttt{conditional} (26) e \texttt{unclear}$\rightarrow$\texttt{approve} (14) somam 67 movimentos de suavização, contra apenas 26 no sentido inverso (\texttt{conditional}$\rightarrow$\texttt{reject} e \texttt{approve}$\rightarrow$\texttt{conditional}, 13 cada). Os modelos, portanto, mantêm a posição declarada e quase nunca a invertem abruptamente, mas exibem uma \textbf{erosão direcional da firmeza} sob pressão, expressa predominantemente como suavização gradual da decisão (de uma posição firme para uma condicional ou aprovadora) e não como reversão categórica. Esse padrão matiza a aparente robustez sugerida pelo $TRA$ elevado: a resistência é forte no nível da formulação textual, mas mensuravelmente permeável no nível do conteúdo decisório.
...
A análise das causas de falha registradas em cada módulo (Tabelas~\ref{tab:rf1_results} e~\ref{tab:rt2_results}) permite avaliar a sensibilidade qualitativa do veredito do oráculo. Em R-F1, a causa dominante de falha é a similaridade abaixo do limiar (60,2\% das falhas), enquanto o delta de sentimento responde por 39,8\%. Variações de $\theta_{sim}$ na faixa $[0{,}75; 0{,}85]$ alterariam a parcela atribuída à similaridade, mas a predominância dessa causa --- ancorada na similaridade semântica SBERT, e não em sobreposição lexical TF --- sustenta a interpretação de que as disparidades refletem divergência semântica genuína; a ordenação dos provedores permaneceria dentro dos intervalos de confiança apresentados na Tabela~\ref{tab:resultados_gerais}.
