...
\begin{enumerate}
    \item \textit{Planejamento:} reconhecimento da necessidade da revisão, definição de objetivos específicos e elaboração do protocolo de investigação, abrangendo perguntas de pesquisa, estratégia de busca, critérios de seleção e avaliação da qualidade dos estudos;
...
\begin{table*}[htb!]
  ...
  \begin{tabular}{|c|p{14.2cm}|}
    ...
    \hline
    \textbf{RQ3} & Que evidências empíricas existem sobre a efetividade, custos, limitações e riscos das abordagens de operacionalização? \\
    \hline
    \textbf{RQ4} & Quais lacunas, desafios e necessidades futuras são apontados, e como a ética em \acrshort{IA} é integrada nas diferentes fases do ciclo de vida do desenvolvimento de software? \\
    \hline
  \end{tabular}
\end{table*}
...
A Tabela~\ref{tab:estatisticas_qualidade_total} apresenta as estatísticas agregadas dos critérios de qualidade para os 1.351 estudos avaliados no Estágio 1. Os valores relativamente baixos refletem a aplicação rigorosa dos critérios em um corpus amplo, resultando em alta taxa de exclusão (93,9\%). Apenas os estudos que alcançaram QA $\geq$ 6,0 nessa etapa preliminar foram selecionados para avaliação completa no Estágio 2.
...
Dos 1.351 estudos aprovados na triagem IC/EC, 83 foram pré-selecionados no Estágio 1 da Avaliação de Qualidade (análise preliminar com base em abstract, QA $\geq$ 6,0), correspondendo a uma taxa de reprovação de 93,9\% (1.268 estudos com QA < 6,0). No Estágio 2, que envolveu a leitura completa dos 83 textos pré-selecionados, 8 estudos permaneceram com o texto completo indisponível, mesmo após tentativas sistemáticas de obtenção via contato com autores e bibliotecas institucionais. Adicionalmente, 18 estudos foram excluídos após leitura integral por não apresentarem a dimensão ética como parte fundamental da pesquisa, tratando-a apenas de forma tangencial ou em seções isoladas de discussão.

Os \textbf{57 estudos restantes} foram incluídos na síntese final (QA $\geq$ 6,5) \todo[inline, color=green]{é preciso colocar a tabela com esses cálculos}. O escore médio de qualidade foi de 7,22 em 8,0 pontos possíveis (DP = 0,40), evidenciando o alto rigor metodológico dos estudos selecionados.
...
A categoria \textit{Fairness} concentra 47 menções (21,7\% do total), refletindo uma preocupação central com a equidade algorítmica e a mitigação de vieses discriminatórios em sistemas de \acrshort{IA} \cite{Mehrabi2021, Chouldechova2017}. Esse princípio manifesta-se por meio de implementações técnicas específicas, como redução de vieses estatísticos, padronização de métricas entre grupos demográficos \cite{Barocas2019, Corbett‑Davies2018} e auditoria de equidade algorítmica \cite{Raji2020, buolamwini2018gender}.
